...
部分 API 功能是重复的，且有部分 API 文档字段定义有一些问题（按 [NatCat 的 OpenAPI](https://napcat.apifox.cn/5430207m0) 整理）。请以 JavaDoc 为准。

绝大部分可能耗时的 API 调用都提供了基于虚拟线程的异步版本。然而基于 `@PrivateMessageListener` 等注解被 NapCat4j 自动调用的方法，本身执行时已经在虚拟线程上了；除非有特殊需求，大多数时候用同步版本的 API 方法即可。
